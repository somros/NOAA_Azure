...
cluster <- doAzureParallel::makeCluster("NOAA_Azure/cluster_OY_MS.json")#, resourceFiles = resource_files)
...
runidx <- 1:52 # 13 f's 4 scenarios
...
opts <- list(maxTaskRetryCount = 0, # do not retry a task if it fails
             maxDate = Sys.time() + 60 * 60 * 24 * 15, # 15 days
             autoDeleteJob=FALSE)
...
atlantis.scenarios <- foreach(idx=runidx, .options.azure=opts, .errorhandling = 'pass', .verbose = TRUE) %dopar% { # move to dopar to run jobs in parallel, do does it on local host
  ...
  pat <- ""
  system(paste0("git clone https://",pat,"@github.com/somros/Atlantis_GOA_OY_MS.git"))
  ...
  system(paste("cd Atlantis_GOA_OY_MS/; flip -uv *; sh ", this_sh, sep=""), wait = TRUE)

  # get the files we need
  outFolder <- paste("Atlantis_GOA_OY_MS/output",sep="_")
  ...
  container <- storage_container(storage_account, "oy-ms")
  ...
  mort <- read.table(mort_path, sep = ' ', header = T)

  # package results 
  list_names <- c("sh", 
                  paste("biomage",idx,sep="_"), 
                  paste("catch",idx,sep="_"), 
                  paste("mort",idx,sep="_"))
  
  results_list <- list(this_sh, biomage, catch, mort)
